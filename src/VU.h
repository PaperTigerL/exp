...
void VU_vprint_f(vsip_vview_f* a) {
    vsip_length i;
    vsip_length n = vsip_vgetlength_f(a); // 获取向量长度

    // 遍历向量中的每个元素
    for(i = 0; i < n; i++) {
        // 打印当前元素，如果当前元素不是最后一个元素，则在其后添加一个空格
        printf("%5.3f%s", vsip_vget_f(a, i), (i < n - 1) ? " " : "");
    }
    printf("\n"); // 在所有元素打印完毕后，打印换行符
    return;
}

void VU_cmprint_f(vsip_cmview_f* a) {
    vsip_length i, j;
    for(i = 0; i < vsip_cmgetcollength_f(a); i++) {
        for(j = 0; j < vsip_cmgetrowlength_f(a); j++) {
            vsip_cscalar_f x = vsip_cmget_f(a, i, j);
            printf("%5.3f + %5.3fi ", x.r, x.i); // 在格式字符串末尾添加空格
        }
        printf("\n");
    }
}
...
/*well I kown It's not right to put them here,but I have to,because I think It's too hard to find the right c to achieve it.*/
